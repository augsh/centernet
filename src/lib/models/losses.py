...
def _neg_loss(pred, gt): # (0, 1]
  ''' Modified focal loss. Exactly the same as CornerNet.
    ...
    Arguments:
      ...
      ddd shpe: [1, 3, 96, 320]
  '''
  pos_inds = gt.eq(1).float()  # positive mask: 等于1的位置为1.0，其他为0.0
  neg_inds = gt.lt(1).float()  # negative mask: 小于1的位置为1.0，其他为0.0

  # negative weights: gt元素值越接近1，则权重越趋于0; 越接近0越趋于1.0
  neg_weights = torch.pow(1 - gt, 4) # 开口向上的抛物线的左侧部分

  loss = 0

  # 二分类问题的交叉熵损失的变形
  pos_loss = torch.log(pred) * torch.pow(1 - pred, 2) * pos_inds # (-∞,0)
  neg_loss = torch.log(1 - pred) * torch.pow(pred, 2) * neg_weights * neg_inds # (0,-∞)
